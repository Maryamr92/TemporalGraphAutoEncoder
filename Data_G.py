...
def generate_temporal_graph_dataset(
    ...
    if visualize and num_samples > 0:
        _visualize_temporal_graph(all_adj_tensors[0], Time, node_i, node_j, selected_t=[0, 3, 6, 9])

    return dataset

def _visualize_temporal_graph(A_tensor, Time, node_i, node_j, selected_t=None):
    """
    ...
    if selected_t is None:
        selected_t = list(range(min(4, Time)))  # Ensure we don't exceed available time steps
